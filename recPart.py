...
def draw_partitions(S, T, parts):
    ...
    for el in parts:
        ...
        if not part_sub.regular_partition:
            ...
            if subs[0] == 1:
                sub_x.append(start_x[-1]+width[-1]/2)
            else:
                for j in range(1, subs[0]):
                    sub_x.append(start_x[-1]+width[-1]/subs[0]*j)
            if subs[1] == 1:
                sub_y.append(start_y[-1]+width[-1]/2)
            else:
                for j in range(1, subs[1]):
                    sub_y.append(start_y[-1]+width[-1]/subs[1]*j)

        for e in sub_x:
            p.line(x=(e,e), y=(start_y[-1],end_y[-1]), line_dash='dashed', line_color="black")

        for e in sub_y:
            p.line(x=(e,e), y=(start_x[-1],end_x[-1]), line_dash='dashed', line_color="black")
